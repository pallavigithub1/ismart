    ...
    <div class="content-wrapper">
      ...
      <div class="content-header">
        <div class="container-fluid">        
          <div class="row mb-2">
            <div class="col-sm-6">
              <h1 class="m-0">Add English Calendar for Endowment Bookings</h1>
            </div><!-- /.col -->
            <div class="col-sm-6">
  ...
  if($(this).val() === '1'){
    if(x.style.display === "none") {
      $.ajax({
        ...
        url:"<?php echo site_url("AddEndowment/event_fetch")?>",
        type:"POST",
        success:function(response) {
          ...
          if(response.result == 1) 
          {
            ...
            for(var i=0; i < response.message.length; i++) {
              mode1 += '<tr>';
              mode1 += '<td style="text-align:center;"><input type="text" class="form-control" name="event[]" value="'+response.message[i].seva_date+'" style="width:130px;" readonly/></td>';
              mode1 += '<td style="text-align:center;"><input type="text" class="form-control booking_start" name="booking_start[]" style="width:230px;"/></td>';
              mode1 += '</tr>';
              $('#dropdown').click(function () { 
                if($(this).val() === '1'){
                  $('#events').empty();
            ...
            mode1 += '</table>';
            mode1 += '<br><br>';
            mode1 += '<button type="submit" id="eventbtn" class="btn btn-primary" style="margin-left:245px;">Submit</button>';
            ...
            $('#form1').submit(function(e){
              ...
              $.ajax({
                type : "POST",
                url : '<?php  echo site_url("AddEndowment/store_event")  ?>',
                data : formData,
                contentType: false,
                processData: false,
                success:function(response){
  ...
  if($(this).val() === '2'){
    if(y.style.display === "none") {
      $.ajax({
        ...
        url:"<?php echo site_url("AddEndowment/panchanga_fetch")?>",
        type:"POST",
        success:function(response) {
          response = jQuery.parseJSON(response);
          console.log(response);
          ...
          if(response.result == 1) 
          {
            ...
            mode1 += '</tr>';
            for (var i=0; i < response.message.length; i++) {
              var Str = response.message[i].seva_date;
              var SplitOn = "/";
              ...
              // for (var j = 0; j < 1; j++) {
                ...
                mode1 += '<td style="text-align:center;"><input type="text" class="form-control panchanga_start" name="panchanga_start[]" style="width:115px;"/></td>';
                mode1 += '</tr>';
                //}
              }
              $('#dropdown').click(function () { 
            ...
            mode1 += '</table>'; 
            mode1 += '<br><br>';
            mode1 += '<button type="submit" id="panchbtn" class="btn btn-primary" style="margin-left:240px;">Submit</button>';
            ...
            $('#form2').submit(function(e){
              ...
              $.ajax({
                type : "POST",
                url : '<?php  echo site_url("AddEndowment/store_panchanga")  ?>',
                data : formData,
                contentType: false,
                processData: false,
                success:function(response){
  ...
  if($(this).val() === '3'){
    if(z.style.display === "none") {
      $.ajax({
        ...
        url:"<?php echo site_url("AddEndowment/english_date_fetch")?>",
        type:"POST",
        success:function(response) {
          response = jQuery.parseJSON(response);
          console.log(response);
          ...
          if(response.result == 1) 
          {
            ...
            for (var i=0; i < response.message.length; i++) {
              ...
              if(Results.length == 3){
                ...
                mode1 += '<td style="text-align:center;"><input type="text" class="form-control english_start" name="english_start[]" style="width:95px;"/></td>';
                mode1 += '</tr>';
              }
              $('#dropdown').click(function () { 
                if($(this).val() === '3'){
            ...
            mode1 += '</table>';
            mode1 += '<br><br>'; 
            mode1 += '<button type="submit" id="timesbtn" class="btn btn-primary" style="margin-left:150px;">Submit</button>';
            ...
            $('#form3').submit(function(e){
              ...
              $.ajax({
                type : "POST",
                url : '<?php  echo site_url("AddEndowment/store_date")  ?>',
                data : formData,
                contentType: false,
                processData: false,
                success:function(response){
